...
extra_name="d.99_dim_64_f_h_t_2"
...
for ((i = start; i <= end_; i++)); do
    python ${main_py_path} --gpu ${gpu} --save_dir ${bash_script_path} --run_group Kitchen_${extra_name}_${current_time} --env_name kitchen-mixed-v0 --project ${project} \
        --seed $((i * 100)) -algo_name ask_hilp --pretrain_steps 500002 --eval_interval 50000 --save_interval 100000 --eval_episodes 24 --num_video_episodes 6 --way_steps 15 \
    --sparse_data 0 --expert_data_On 0 --spherical_On 1.0 --rep_type concat --mapping_threshold 0.0 --rep_normalizing_On 1 --rep_dim 10 --hilp_skill_dim 64 --build_keynode_time during_training --keynode_num 500 --kmean_weight_On 0 --use_goal_info_On 0 --kmean_weight_type rtg_uniform --specific_dim_On 1 --keynode_ratio 0.0 --use_keynode_in_eval_On 0 \
    --relative_dist_in_eval_On 1 --mapping_method nearest --kl_loss ${kl_loss} --final_goal ${final_goal} --mse_loss ${mse_loss} --high_action_in_hilp ${high_action_in_hilp} --low_actor_train_with_high_actor ${low_actor_train_with_high_actor} --correction_value ${correction_value} --n_step_hilp ${n_step_hilp} --high_temperature ${high_temperature} --temperature ${temperature} --discount ${discount} --pretrain_expectile ${pretrain_expectile} --distance ${distance}
