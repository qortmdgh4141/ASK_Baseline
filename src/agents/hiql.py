...
def compute_actor_loss(agent, batch, network_params):
    ...
    #             # observations = batch['observations'][:,:3]
    #         else:
    #             cur_goals = cur_goals[:,3:6]
    #             # observations = batch['observations'][:,3:6]
    #     cur_goals = cur_goals[:,:3]
    dist = agent.network(batch['observations'], cur_goals, state_rep_grad=True, goal_rep_grad=False, method='actor', params=network_params)
...
def create_learner(
        ...
        **kwargs):
        ...
        if visual:
            ...
            def make_encoder(bottleneck):
                if bottleneck:
                    return RelativeRepresentation(rep_dim=rep_dim, hidden_dims=(*value_hidden_dims, rep_dim), layer_norm=use_layer_norm, rep_type=flag.rep_type, bottleneck=True)
                else:
                    return RelativeRepresentation(rep_dim=value_hidden_dims[-1], hidden_dims=(*value_hidden_dims, value_hidden_dims[-1]), layer_norm=use_layer_norm, rep_type=flag.rep_type, bottleneck=False)
        ...
        if 'Fetch' in flag.env_name and flag.small_subgoal_space:
