...
def plot_value_map(agent, base_observation, goal_info, i, g_start_time, pretrain_batch, obs, transition_index=None, trajs=None, key_node=None):
    ...
    value = agent.network(observations, goal_infos, method='hilp_value')[0][0].transpose(1,0)
    ...
    if key_node is not None:
        ...
        real_obs_value = agent.network(pretrain_batch['observations'], np.tile(goal_info, (batch_size,1)), method='hilp_value')[0][0]
    ...
    if trajs is not None and subgoals.shape[0]>batch_size:
        ...
        subgoals_value = agent.network(subgoals, np.tile(goal_info, (subgoals.shape[0],1)), method='hilp_value')[0][0]
    ...
    if 'networks_hilp_value' in agent.network.params.keys():
        ...
        hilp_value = agent.network(hilp_filtered_obs_sub, np.tile(goal_info, (hilp_filtered_obs_sub.shape[0],1)), method='hilp_value')[0][0]
        obs_value = agent.network(random_obs_sub, np.tile(goal_info, (random_obs_sub.shape[0],1)), method='hilp_value')[0][0]
...
def plot_value_map_others(agent, base_observation, goal_info, i, g_start_time, pretrain_batch, obs, transition_index=None, trajs=None, key_node=None):
    if trajs is not None:
        subgoals = []
        for j, t in enumerate(trajs):
            subgoals.extend(t['cur_obs_subgoal'])
    ...
    try:
        ...
        print('no tsnecuda module')
        
    X_embedded = TSNE(n_components=2, perplexity=15, learning_rate=10).fit_transform(np.concatenate((pretrain_batch['observations'], key_node.centroids)))
    x, y = X_embedded[:,0], X_embedded[:,1]
    value, _ = agent.network(pretrain_batch['observations'], pretrain_batch['goal_infos'], method='hilp_value')[0]
    
    # value map
    sc1 = axes[0,0].scatter(x,y, c=value, cmap='Blues_r', interpolation='nearest')
    axes[0,0].set_title('all map value')
    axes[0,0].invert_yaxis() 
    ...
    if key_node is not None:
        key_node_index = np.random.choice(pretrain_batch['observations'].shape[0], size=1)
        key_x, key_y = X_embedded[key_node_index,0], X_embedded[key_node_index,1]
        # max_x, max_y = np.max(pretrain_batch['observations'], axis=0)[0], np.max(pretrain_batch['observations'], axis=0)[1]
        sc2 = axes[0,1].scatter(key_x, key_y)
        ...
        # nearest key node map
        batch_size, obs_dim = pretrain_batch['observations'].shape
        real_obs_value, _ = agent.network(pretrain_batch['observations'], np.tile(goal_info, (batch_size,1)), method='hilp_value')[0]
        x, y = pretrain_batch['observations'][:,0], pretrain_batch['observations'][:,1]
        sc2 = axes[0,1].scatter(x, y, c=real_obs_value, cmap=cmap, vmin=-101, vmax=0)
        axes[0,1].set_title('real obs value')
    ...
    if trajs is not None and subgoals.shape[0]>batch_size:
        ...
        subgoals_value, _ = agent.network(subgoals, np.tile(goal_info, (subgoals.shape[0],1)), method='hilp_value')[0]
        x, y = subgoals[:,0], subgoals[:,1]
        sc3 = axes[1,0].scatter(x, y, c=subgoals_value, cmap=cmap, vmin=-101, vmax=0)
        axes[1,0].set_title('subgoals value')
    ...
    if 'networks_hilp_value' in agent.network.params.keys():
        ...
        hilp_value, _ = agent.network(hilp_filtered_obs_sub, np.tile(goal_info, (hilp_filtered_obs_sub.shape[0],1)), method='hilp_value')[0]
        obs_value, _ = agent.network(random_obs_sub, np.tile(goal_info, (random_obs_sub.shape[0],1)), method='hilp_value')[0]
...
def plot_q_map_modified(agent, base_observation, goal_info, i, g_start_time, pretrain_batch, transition_index=None, trajs=None):
    if trajs is not None:
        subgoals = []
        obs = []
        for j, t in enumerate(trajs):
        ...
        subgoals = np.array(subgoals).reshape(-1,32)
        obs = np.array(obs).reshape(-1,32)
    
    base_observation = jnp.concatenate([jnp.array([15,15]) , base_observation[2:]], axis=0)
    ...
    rep_observations = agent.get_hilp_phi(observations=observations)
    
    q_value = agent.network(base_observation_tiled, rep_observations, goal_infos, method='high_qf')[0].transpose(1,0)
    ...
    batch_size, obs_dim = pretrain_batch['observations'].shape
    # real obs value map
    real_obs_value = agent.network(np.tile(base_observation, (batch_size,1)), pretrain_batch['rep_observations'], np.tile(goal_info, (batch_size,1)), method='high_qf')[0]
    x, y = pretrain_batch['observations'][:,0], pretrain_batch['observations'][:,1]
    ...
    obs_min = jnp.min(pretrain_batch['rep_observations'], axis=0)
    obs_max = jnp.max(pretrain_batch['rep_observations'], axis=0)
    random_obs_sub = jax.random.uniform(key=agent.rng, shape=(batch_size, pretrain_batch['rep_observations'].shape[-1]), minval=obs_min, maxval=obs_max)
    ...
    # random_obs_value = agent.network(np.tile(base_observation, (batch_size,1)), pretrain_batch['rep_key_node'], np.tile(goal_info, (random_obs_sub.shape[0],1)), method='high_qf')[0]
...
def plot_guider_q_map(agent, base_observation, goal_info, i, g_start_time, pretrain_batch, transition_index=None, trajs=None):
    ...
    if trajs is not None:
        ...
        obs = []
        for j, t in enumerate(trajs):
            subgoals.extend(t['cur_obs_subgoal'][:,:2])
            obs.extend(t['observation'][:,:2])
        subgoals = np.array(subgoals).reshape(-1,2)
    ...
    # joint = np.tile(base_observation[2:], (59,46,1))
    goal_infos = np.tile(goal_info, (59,46,1))
    base_observation_tiled = np.tile(observations, (59,46,1))
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    from matplotlib.gridspec import GridSpec
    gs = GridSpec(2, 3, width_ratios=[1, 1, 0.05], wspace=0.8)
    cmap = plt.cm.bwr
    ...
    # observations = np.concatenate((coordinates, joint), axis=2)
    # observations = np.concatenate((coordinates, joint), axis=2)
    z, _, _ = agent.get_latent(observations=base_observation_tiled, targets=coordinates, seed=agent.rng)
    
    # q_value = agent.network(base_observation_tiled, z, goal_infos, method='high_qf')[0]
    q_value = agent.network(base_observation_tiled, z, goal_infos, method='high_qf')[0].transpose(1,0)
    ...
    batch_size, obs_dim = pretrain_batch['observations'].shape
    # real obs value map
    real_z, _, _ = agent.get_latent(observations=np.tile(observations, (batch_size,1)), targets=pretrain_batch['observations'][:,:2], seed=agent.rng)
    real_obs_value = agent.network(np.tile(observations, (batch_size,1)), real_z, np.tile(goal_info, (batch_size,1)), method='high_qf')[0]
    x, y = pretrain_batch['observations'][:,0], pretrain_batch['observations'][:,1]
    sc2 = axes[0,1].scatter(x, y, c=real_obs_value, cmap=cmap)
    axes[0,1].set_title('real obs q value')
    ...
    if trajs is not None and subgoals.shape[0]>batch_size:
        ...
        real_z, _, _ = agent.get_latent(observations=np.tile(subgoals, (batch_size,1)), targets=pretrain_batch['observations'][:,:2], seed=agent.rng)
    ...
    obs_min = jnp.min(pretrain_batch['observations'][:,:2], axis=0)
    obs_max = jnp.max(pretrain_batch['observations'][:,:2], axis=0)
    random_obs_sub = jax.random.uniform(key=agent.rng, shape=(batch_size, 2), minval=obs_min, maxval=obs_max)
    
    
    random_z, _, _ = agent.get_latent(observations=np.tile(observations, (batch_size,1)), targets=random_obs_sub, seed=agent.rng)
    
    obs_value = agent.network(np.tile(observations, (batch_size,1)), random_z, np.tile(goal_info, (random_obs_sub.shape[0],1)), method='high_qf')[0]
    
    x_, y_ = random_obs_sub[:,0], random_obs_sub[:,1]
    # key_x, key_y = pretrain_batch['key_node'][:,0], pretrain_batch['key_node'][:,1]
    # random_obs_value = agent.network(np.tile(observations, (batch_size,1)), pretrain_batch['key_node'], np.tile(goal_info, (random_obs_sub.shape[0],1)), method='high_qf')[0]
